...
@Component({
  selector: 'app-estructura-decision',
  templateUrl: './estructura-decision.component.html',
  styleUrls: ['./estructura-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EstructuraDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_estructura_decision');
  ...
  updateAllNotes() {
    ...
    let notes = document.querySelectorAll('app-note-estructura-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_estructura_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_estructura',this.notes_all);
    localStorage.setItem('notes_all_estructura', JSON.stringify(this.notes_all));

    let estructur: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      estructur.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": estructur});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase39', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaEstructura(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase39');

          this.socketWebService.emitEventTableroSaveEstructura({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase39']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateEstructura({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdateEstructura(newValue);
        this._proyectsService.updateEstructura(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroEstructura({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteEstructura(note);
        ...
        localStorage.setItem('notes_estructura_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteEstructura(id)
        .subscribe(
            data => {
