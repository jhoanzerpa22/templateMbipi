...
export class ClientesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_clientes');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_clientes',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-clientes'));
    let notes = document.querySelectorAll('app-note-clientes');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_clientes', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase24', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaClientes(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase24');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase24']);
  }
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createClientes(data)
        .subscribe(
            data => {
    ...
    this.socketWebService.emitEventTableroUpdateClientes({id: id, content: event.target.value, usuario_id: this.usuario.id });
