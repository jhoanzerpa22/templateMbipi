...
@Component({
  selector: 'app-objetivos-corto-decision',
  templateUrl: './objetivos-corto-decision.component.html',
  styleUrls: ['./objetivos-corto-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ObjetivosCortoDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let objetivo: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateObjetivos(this.notes[j].id, objetivo)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_objetivos_corto_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].scopecanvas_objetivo != null){
                if(/*this.proyecto.proyecto_recursos[c].usuario_id == this.usuario.id*/this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.tipo == 'Corto'){
                 objetivos_corto.push({'id': this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.id,'content': this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.contenido, 'type': this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.tipo, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.position ? this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_objetivo.dragPosition) : {x: 0, y: 0});
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-objetivos-corto-decision'));
    let notes = document.querySelectorAll('app-note-objetivos-corto-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_objetivos_corto_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_objetivos_corto',this.notes_all);
    localStorage.setItem('notes_all_objetivos_corto', JSON.stringify(this.notes_all));

    let objetivos_corto: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      objetivos_corto.push({'content': this.notes_all[n].content, 'type': 'Corto'});
    }

    tablero.push({'title': 'Como podriamos', "data": objetivos_corto});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase17', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaObjetivos(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase17');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase17']);
  }
  ...
  addNote(event?: any) {
    this.notes.push({ id: /*this.notes.length + 1*/this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Corto', usuario_id: this.usuario.id });
    // sort the array
    this.notes= this.notes.sort((a: any,b: any)=>{ return b.id-a.id});
    localStorage.setItem('notes_objetivos_corto_decision', JSON.stringify(this.notes));
    
    this.socketWebService.emitEventTableroUpdateObjetivos({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Corto', usuario_id: this.usuario.id });
