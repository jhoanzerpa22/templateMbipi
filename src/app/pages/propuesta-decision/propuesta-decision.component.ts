...
@Component({
  selector: 'app-propuesta-decision',
  templateUrl: './propuesta-decision.component.html',
  styleUrls: ['./propuesta-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PropuestaDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let propuesta: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updatePropuesta(this.notes[j].id, propuesta)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroPropuesta.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_propuesta_decision');
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_propuestum
                != null){
                 ...
                 .contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_propuestum.position ? this.proyecto.proyecto_recursos[c].leancanvas_propuestum.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_propuestum.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_propuestum.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_propuestum.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_propuestum.dragPosition) : {x: 0, y: 0});
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_propuesta_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-propuesta-decision'));
    let notes = document.querySelectorAll('app-note-propuesta-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_propuesta_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_propuesta',this.notes_all);
    localStorage.setItem('notes_all_propuesta', JSON.stringify(this.notes_all));

    let propuesta: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      propuesta.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": propuesta});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase33', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaPropuesta(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase33');

          this.socketWebService.emitEventTableroSavePropuesta({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase33']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdatePropuesta({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdatePropuesta(newValue);
        this._proyectsService.updatePropuesta(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroPropuesta({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroPropuesta({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeletePropuesta(note);
        ...
        localStorage.setItem('notes_propuesta_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deletePropuesta(id)
        .subscribe(
            data => {
