...
export class SolucionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_solucion');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_solucion',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-solucion'));
    let notes = document.querySelectorAll('app-note-solucion');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_solucion', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase28', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaSolucion(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase28');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase28']);
  }
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createSolucion(data)
        .subscribe(
            data => {
    const id = data.solucion_id;
    ...
    this.socketWebService.emitEventTableroUpdateSolucion({id: id, content: event.target.value, usuario_id: this.usuario.id });
