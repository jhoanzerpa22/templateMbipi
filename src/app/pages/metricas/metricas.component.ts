...
export class MetricasComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_metricas');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_metricas',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-metricas'));
    let notes = document.querySelectorAll('app-note-metricas');
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase22', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaMetricas(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase22');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase22']);
  }
