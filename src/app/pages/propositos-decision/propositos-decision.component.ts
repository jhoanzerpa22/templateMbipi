...
@Component({
  selector: 'app-propositos-decision',
  templateUrl: './propositos-decision.component.html',
  styleUrls: ['./propositos-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PropositosDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let propositos: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updatePropositos(this.notes[j].id, propositos)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroPropositos.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_propositos_decision');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_propositos_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-propositos-decision'));
    let notes = document.querySelectorAll('app-note-propositos-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_propositos_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_propositos',this.notes_all);
    localStorage.setItem('notes_all_propositos', JSON.stringify(this.notes_all));
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase15', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaPropositos(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase15');

          this.socketWebService.emitEventTableroSavePropositos({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase15']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdatePropositos({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroPropositos({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeletePropositos(note);
        ...
        localStorage.setItem('notes_propositos_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deletePropositos(id)
        .subscribe(
            data => {
