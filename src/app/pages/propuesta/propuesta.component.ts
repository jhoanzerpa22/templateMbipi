...
export class PropuestaComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_propuesta');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_propuesta',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-propuesta'));
    let notes = document.querySelectorAll('app-note-propuesta');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_propuesta', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    for(let n in this.notes_all){
      propuest.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": propuest});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase32', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaPropuesta(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase32');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase32']);
  }
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createPropuesta(data)
        .subscribe(
            data => {
    const id = data.propuesta_id;
    ...
    this.socketWebService.emitEventTableroUpdatePropuesta({id: id, content: event.target.value, usuario_id: this.usuario.id });
