...
export class ClientesDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let cliente: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateClientes(this.notes[j].id, cliente)
    .subscribe(
        data => {
        },
        (response) => {
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_cliente != null){
                 ...
                 clientes.push({'id': this.proyecto.proyecto_recursos[c].leancanvas_cliente.id,'content': this.proyecto.proyecto_recursos[c].leancanvas_cliente.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_cliente.position ? this.proyecto.proyecto_recursos[c].leancanvas_cliente.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_cliente.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_cliente.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_cliente.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_cliente.dragPosition) : {x: 0, y: 0});
