...
@Component({
  selector: 'app-problema-decision',
  templateUrl: './problema-decision.component.html',
  styleUrls: ['./problema-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ProblemaDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let problema: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateProblema(this.notes[j].id, problema)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_problema_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_problema != null){
                 ...
                 problema.push({'id': this.proyecto.proyecto_recursos[c].leancanvas_problema.id,'content': this.proyecto.proyecto_recursos[c].leancanvas_problema.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_problema.position ? this.proyecto.proyecto_recursos[c].leancanvas_problema.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_problema.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_problema.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_problema.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_problema.dragPosition) : {x: 0, y: 0});
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_problema_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-problema-decision'));
    let notes = document.querySelectorAll('app-note-problema-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_problema_decision', JSON.stringify(this.notes));
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateProblema({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
