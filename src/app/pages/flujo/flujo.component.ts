...
export class FlujoComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_flujo');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_flujo',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-flujo'));
    let notes = document.querySelectorAll('app-note-flujo');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_flujo', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase40', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaFlujo(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase40');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase40']);
  }
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createFlujo(data)
        .subscribe(
            data => {
    const id = data.flujo_id;
    ...
    this.socketWebService.emitEventTableroUpdateFlujo({id: id, content: event.target.value, usuario_id: this.usuario.id });
