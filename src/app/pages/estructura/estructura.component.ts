...
export class EstructuraComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_estructura');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_estructura',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-estructura'));
    let notes = document.querySelectorAll('app-note-estructura');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_estructura', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase38', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaEstructura(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase38');
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createEstructura(data)
        .subscribe(
            data => {
    const id = data.estructura_id;
    ...
    this.socketWebService.emitEventTableroUpdateEstructura({id: id, content: event.target.value, usuario_id: this.usuario.id });
