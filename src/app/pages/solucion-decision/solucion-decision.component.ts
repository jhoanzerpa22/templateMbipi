...
@Component({
  selector: 'app-solucion-decision',
  templateUrl: './solucion-decision.component.html',
  styleUrls: ['./solucion-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SolucionDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let solucion: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateSolucion(this.notes[j].id, solucion)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroSolucion.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_solucion_decision');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_solucion_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-solucion-decision'));
    let notes = document.querySelectorAll('app-note-solucion-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_solucion_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_solucion',this.notes_all);
    localStorage.setItem('notes_all_solucion', JSON.stringify(this.notes_all));

    let solucio: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      solucio.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": solucio});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase29', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaSolucion(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase29');

          this.socketWebService.emitEventTableroSaveSolucion({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase29']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateSolucion({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdateSolucion(newValue);
        this._proyectsService.updateSolucion(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroSolucion({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroSolucion({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteSolucion(note);
        ...
        localStorage.setItem('notes_solucion_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteSolucion(id)
        .subscribe(
            data => {
