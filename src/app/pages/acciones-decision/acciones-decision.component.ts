...
@Component({
  ...
  templateUrl: './acciones-decision.component.html',
  styleUrls: ['./acciones-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AccionesDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let accion: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateAcciones(this.notes[j].id, accion)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroAcciones.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_acciones_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].scopecanvas_accione != null){
                 acciones.push({'id': this.proyecto.proyecto_recursos[c].scopecanvas_accione.id,'content': this.proyecto.proyecto_recursos[c].scopecanvas_accione.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].scopecanvas_accione.position ? this.proyecto.proyecto_recursos[c].scopecanvas_accione.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].scopecanvas_accione.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_accione.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].scopecanvas_accione.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_accione.dragPosition) : {x: 0, y: 0});
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-acciones-decision'));
    let notes = document.querySelectorAll('app-note-acciones-decision');
  ...
  saveNoteAll() {
    console.log('save_notes_all_acciones',this.notes_all);
    localStorage.setItem('notes_all_acciones', JSON.stringify(this.notes_all));

    let acciones: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      acciones.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": acciones});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase21', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaAcciones(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase21');

          this.socketWebService.emitEventTableroSaveAcciones({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase21']);
  }
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroAcciones({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroAcciones({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteAcciones(note);
        ...
        localStorage.setItem('notes_acciones_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteAcciones(id)
        .subscribe(
            data => {
