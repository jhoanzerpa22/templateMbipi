...
@Component({
  ...
  templateUrl: './metricas-clave.component.html',
  styleUrls: ['./metricas-clave.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MetricasClaveComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_metricas_clave');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_metricas_clave',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-metricas_clave'));
    let notes = document.querySelectorAll('app-note-metricas_clave');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_metricas_clave', JSON.stringify(this.notes));
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createMetricasClave(data)
        .subscribe(
            data => {
    const id = data.metricas_clave_id;
    ...
    this.socketWebService.emitEventTableroUpdateMetricasClave({id: id, content: event.target.value, usuario_id: this.usuario.id });
