...
@Component({
  selector: 'app-metricas-decision',
  templateUrl: './metricas-decision.component.html',
  styleUrls: ['./metricas-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MetricasDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let metrica: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateMetricas(this.notes[j].id, metrica)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroMetricas.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_metricas_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].scopecanvas_metrica != null){
                 ...
                 metricas.push({'id': this.proyecto.proyecto_recursos[c].scopecanvas_metrica.id,'content': this.proyecto.proyecto_recursos[c].scopecanvas_metrica.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].scopecanvas_metrica.position ? this.proyecto.proyecto_recursos[c].scopecanvas_metrica.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].scopecanvas_metrica.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_metrica.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].scopecanvas_metrica.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].scopecanvas_metrica.dragPosition) : {x: 0, y: 0});
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-metricas-decision'));
    let notes = document.querySelectorAll('app-note-metricas-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_metricas_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_metricas',this.notes_all);
    localStorage.setItem('notes_all_metricas', JSON.stringify(this.notes_all));

    let metricas: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      metricas.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": metricas});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase23', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaMetricas(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase23');

          this.socketWebService.emitEventTableroSaveMetricas({tablero: JSON.stringify(tablero)});
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateMetricas({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroMetricas({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroMetricas({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteMetricas(note);
        ...
        localStorage.setItem('notes_metricas_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteMetricas(id)
        .subscribe(
            data => {
