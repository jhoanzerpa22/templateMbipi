...
export class VentajasComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_ventajas');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_ventajas',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-ventajas'));
    let notes = document.querySelectorAll('app-note-ventajas');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_ventajas', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase34', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaVentajas(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase34');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase34']);
  }
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createVentajas(data)
        .subscribe(
            data => {
    const id = data.ventajas_id;
    ...
    this.socketWebService.emitEventTableroUpdateVentajas({id: id, content: event.target.value, usuario_id: this.usuario.id });
