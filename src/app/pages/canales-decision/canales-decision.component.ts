...
@Component({
  selector: 'app-canales-decision',
  templateUrl: './canales-decision.component.html',
  styleUrls: ['./canales-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class CanalesDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_canales_decision');
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-canales-decision'));
    let notes = document.querySelectorAll('app-note-canales-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_canales_decision', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase37', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaCanales(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase37');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase37']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateCanales({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
