...
export class AccionesComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_acciones');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_acciones',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-acciones'));
    let notes = document.querySelectorAll('app-note-acciones');
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase20', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaAcciones(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase20');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase20']);
  }
