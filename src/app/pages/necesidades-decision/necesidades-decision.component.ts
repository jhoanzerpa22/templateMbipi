...
@Component({
  selector: 'app-necesidades-decision',
  templateUrl: './necesidades-decision.component.html',
  styleUrls: ['./necesidades-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class NecesidadesDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_necesidades_decision');
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-necesidades-decision'));
    let notes = document.querySelectorAll('app-note-necesidades-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_necesidades_decision', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase11', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaNecesidades(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase11');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase11']);
  }
  ...
  addNote(event?: any) {
    this.notes.push({ id: /*this.notes.length + 1*/this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Motivadores', usuario_id: this.usuario.id });
    // sort the array
    this.notes= this.notes.sort((a: any,b: any)=>{ return b.id-a.id});
    localStorage.setItem('notes_necesidades_decision', JSON.stringify(this.notes));
    
    this.socketWebService.emitEventTableroUpdateNecesidades({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Motivadores', usuario_id: this.usuario.id });
