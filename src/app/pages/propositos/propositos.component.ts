...
@Component({
  selector: 'app-propositos',
  templateUrl: './propositos.component.html',
  styleUrls: ['./propositos.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PropositosComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_propositos');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_propositos',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-propositos'));
    let notes = document.querySelectorAll('app-note-propositos');
  ...
  saveNoteAll() {
    ...
    for(let n in this.notes_all){
      propositos.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": propositos});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase14', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaPropositos(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase14');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase14']);
  }
