...
@Component({
  ...
  templateUrl: './metricas-clave-decision.component.html',
  styleUrls: ['./metricas-clave-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class MetricasClaveDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    this._proyectsService.updateMetricasClave(this.notes[j].id, metrica)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroMetricasClave.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_metricas_clave_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_metrica != null){
                 ...
                 metricas_clave.push({'id': this.proyecto.proyecto_recursos[c].leancanvas_metrica.id,'content': this.proyecto.proyecto_recursos[c].leancanvas_metrica.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_metrica.position ? this.proyecto.proyecto_recursos[c].leancanvas_metrica.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_metrica.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_metrica.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_metrica.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_metrica.dragPosition) : {x: 0, y: 0});
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_metricas_clave_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-metricas-clave-decision'));
    let notes = document.querySelectorAll('app-note-metricas-clave-decision');
  ...
  saveNoteAll() {
    console.log('save_notes_all_metricas_clave',this.notes_all);
    localStorage.setItem('notes_all_metricas_clave', JSON.stringify(this.notes_all));

    let metricas_clav: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      metricas_clav.push({'content': this.notes_all[n].content});
    }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateMetricasClave({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdateMetricasClave(newValue);
        this._proyectsService.updateMetricasClave(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroMetricasClave({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroMetricasClave({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteMetricasClave(note);
        ...
        localStorage.setItem('notes_metricas_clave_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteMetricasClave(id)
        .subscribe(
            data => {
