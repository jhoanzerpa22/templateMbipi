...
export class RepasoComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase43'};

    this._proyectsService.updateEtapaBosquejar(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase43');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase43']);
  }
