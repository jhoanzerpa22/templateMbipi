...
@Component({
  selector: 'app-ventajas-decision',
  templateUrl: './ventajas-decision.component.html',
  styleUrls: ['./ventajas-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class VentajasDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let ventaja: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateVentajas(this.notes[j].id, ventaja)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroVentajas.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_ventajas_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_ventaja != null){
                 ...
                 ventajas.push({'id': this.proyecto.proyecto_recursos[c].leancanvas_ventaja.id,'content': this.proyecto.proyecto_recursos[c].leancanvas_ventaja.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_ventaja.position ? this.proyecto.proyecto_recursos[c].leancanvas_ventaja.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_ventaja.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_ventaja.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_ventaja.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_ventaja.dragPosition) : {x: 0, y: 0});
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_ventajas_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-ventajas-decision'));
    let notes = document.querySelectorAll('app-note-ventajas-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_ventajas_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_ventajas',this.notes_all);
    localStorage.setItem('notes_all_ventajas', JSON.stringify(this.notes_all));

    let ventaja: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      ventaja.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": ventaja});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase35', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaVentajas(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase35');

          this.socketWebService.emitEventTableroSaveVentajas({tablero: JSON.stringify(tablero)});
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase35']);
  }
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateVentajas({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdateVentajas(newValue);
        this._proyectsService.updateVentajas(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroVentajas({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroVentajas({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteVentajas(note);
        ...
        localStorage.setItem('notes_ventajas_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteVentajas(id)
        .subscribe(
            data => {
