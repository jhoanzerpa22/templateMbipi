...
@Component({
  selector: 'app-flujo-decision',
  templateUrl: './flujo-decision.component.html',
  styleUrls: ['./flujo-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class FlujoDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  public onDragEnded(event: CdkDragEnd<any>, i?: any, j?: any){
    ...
    let flujo: any = { 'position': this.notes[j].position, 'dragPosition': this.notes[j].dragPosition };

    this._proyectsService.updateFlujo(this.notes[j].id, flujo)
    .subscribe(
        data => {
        },
        (response) => {
  ...
  private route: ActivatedRoute
    ...
    this.socketWebService.outEvenTableroFlujo.subscribe((res: any) => {
      const { tablero } = res;
    ...
    const notes: any = localStorage.getItem('notes_flujo_decision');
          ...
          (response) => {
            ...
            for(let c in this.proyecto.proyecto_recursos){
              if(this.proyecto.proyecto_recursos[c].leancanvas_flujo != null){
                 ...
                 flujo.push({'id': this.proyecto.proyecto_recursos[c].leancanvas_flujo.id,'content': this.proyecto.proyecto_recursos[c].leancanvas_flujo.contenido, 'usuario_id': this.proyecto.proyecto_recursos[c].usuario_id, 'position': this.proyecto.proyecto_recursos[c].leancanvas_flujo.position ? this.proyecto.proyecto_recursos[c].leancanvas_flujo.position : position, 'dragPosition': this.proyecto.proyecto_recursos[c].leancanvas_flujo.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_flujo.dragPosition) :  {'x': 0, 'y': 0}});

                 this.dragPosition.push(this.proyecto.proyecto_recursos[c].leancanvas_flujo.dragPosition ? JSON.parse(this.proyecto.proyecto_recursos[c].leancanvas_flujo.dragPosition) : {x: 0, y: 0});
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_flujo_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-flujo-decision'));
    let notes = document.querySelectorAll('app-note-flujo-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_flujo_decision', JSON.stringify(this.notes));

  }

  saveNoteAll() {
    console.log('save_notes_all_flujo',this.notes_all);
    localStorage.setItem('notes_all_flujo', JSON.stringify(this.notes_all));

    let fluj: any = [];
    let tablero: any = [];
    for(let n in this.notes_all){
      fluj.push({'content': this.notes_all[n].content});
    }

    tablero.push({'title': 'Como podriamos', "data": fluj});

    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase41', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaFlujo(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase41');

          this.socketWebService.emitEventTableroSaveFlujo({tablero: JSON.stringify(tablero)});
  ...
  addNote(event?: any) {
    ...
    this.socketWebService.emitEventTableroUpdateFlujo({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this.socketWebService.emitEventTableroUpdateFlujo(newValue);
        this._proyectsService.updateFlujo(newValue.id,newValue)
        .subscribe(
            data => {
  ...
  updateNoteAll(newValue: any){
    ...
    this.socketWebService.emitEventTableroFlujo({tablero: JSON.stringify(this.notes_all)});
  }
  ...
  sendNotes(notes: any){
    this.socketWebService.emitEventTableroFlujo({tablero: JSON.stringify(notes)});
  }
  ...
  deleteNote(event: any, idNote: any){
    ...
    this.notes.forEach((note: any, index: any)=>{
      ...
      if(note.id== id) {
        ...
        this.socketWebService.emitEventTableroDeleteFlujo(note);
        ...
        localStorage.setItem('notes_flujo_decision', JSON.stringify(this.notes));
        if(id > 0){
        this._proyectsService.deleteFlujo(id)
        .subscribe(
            data => {
