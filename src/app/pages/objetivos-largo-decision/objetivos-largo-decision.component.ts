...
@Component({
  selector: 'app-objetivos-largo-decision',
  templateUrl: './objetivos-largo-decision.component.html',
  styleUrls: ['./objetivos-largo-decision.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ObjetivosLargoDecisionComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private route: ActivatedRoute
    ...
    const notes: any = localStorage.getItem('notes_objetivos_largo_decision');
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_objetivos_largo_decision',data);
    this.notes_all = data;
    this.notes = data;
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-objetivos-largo-decision'));
    let notes = document.querySelectorAll('app-note-objetivos-largo-decision');

    notes.forEach((note: any, index: any)=>{
         this.notes[note.id].content = note.querySelector('.content').innerHTML;
    });

    localStorage.setItem('notes_objetivos_largo_decision', JSON.stringify(this.notes));
  ...
  saveNoteAll() {
    ...
    const data_etapa = {etapa_activa: '/proyect-init/'+this.proyecto_id+'/fase19', tablero: this.notes_all, type: 'notas'};

    this._proyectsService.updateEtapaObjetivos(this.proyecto_id, data_etapa)
    .subscribe(
        data => {

          this.socketWebService.emitEventSetEtapa('/proyect-init/'+this.proyecto_id+'/fase19');
    ...
    //this._router.navigate(['/proyect-init/'+this.proyecto_id+'/fase19']);
  }
  ...
  addNote(event?: any) {
    this.notes.push({ id: /*this.notes.length + 1*/this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Largo', usuario_id: this.usuario.id });
    // sort the array
    this.notes= this.notes.sort((a: any,b: any)=>{ return b.id-a.id});
    localStorage.setItem('notes_objetivos_largo_decision', JSON.stringify(this.notes));
    
    this.socketWebService.emitEventTableroUpdateObjetivos({id: this.notes.length+'-'+this.usuario.nombre, content: event.target.value, type: 'Largo', usuario_id: this.usuario.id });
