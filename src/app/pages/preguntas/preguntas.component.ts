...
@Component({
  ...
  styleUrls: ['./preguntas2.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class PreguntasComponent implements OnInit, AfterViewInit, OnDestroy {
  // Public variables
  selfLayout = 'default';
  ...
  private readBoard(tablero: any, emit: boolean){
    ...
    console.log('notas_all_preguntas',data);
    this.notes_all = data;
    /*this.notes_all = [];
    for(let c in data){
  ...
  updateAllNotes() {
    console.log(document.querySelectorAll('app-note-pregunta'));
    let notes = document.querySelectorAll('app-note-pregunta');
  ...
  addNote(event?: any) {
    ...
    this._proyectsService.createPreguntaSprint(data)
        .subscribe(
            data => {
    const id = data.pregunta_id;
    ...
    this.socketWebService.emitEventTableroUpdatePreguntas({id: id, content: event.target.value, usuario_id: this.usuario.id });
  ...
  buttonNote() {
    ...
    this.socketWebService.emitEventTableroUpdatePreguntas({id: id, content: nota, usuario_id: this.usuario.id });
  ...
  updateNote(newValue: any){
    this.notes.forEach((note: any, index: any)=>{
      if(note.id== newValue.id) {
        ...
        this._proyectsService.updatePreguntaSprint(newValue.id,newValue)
        .subscribe(
            data => {
